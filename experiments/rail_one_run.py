...
- builds four built-in benchmark-like datasets:
    * Synthetic
    * SECOM-like
    * APS-like
    * ATC-like
- trains a simple online probabilistic classifier
- calibrates the three gated baselines to match the validation-window admitted yield
  of RAIL-Gated
- runs one replay evaluation pass per dataset
- writes CSV, JSON, and LaTeX tables

Important
- This script is meant for honest, reproducible evaluation.
- It does not force any method to win.
"""
...
def score_margin_gated(probs: np.ndarray) -> float:
    ...
    if len(p) == 1:
        return 1.0
    return float(np.clip(p[0] - p[1], 0.0, 1.0))
...
def run_all_policies_once(
    ...
    always_metrics = replay_once(dataset_name, base_model, events, X_test, y_test, by_name["always"])
    ...
    for p in policies:
        if p.name.lower() == "always":
            continue
        out.append(replay_once(dataset_name, base_model, events, X_test, y_test, p, always_reference_counts=ref))

    order = [p.name for p in policies]
    return sorted(out, key=lambda r: order.index(r.method))


def _fmt(mean: float, std: float | None = None, decimals: int = 3) -> str:
    if std is None:
        return f"{mean:.{decimals}f}"
    return f"{mean:.{decimals}f} $\\pm$ {std:.{decimals}f}"


def latex_main_table(rows: Sequence[RunMetrics], datasets: Sequence[str], methods: Sequence[str]) -> str:
    rows_by_key = {(r.dataset, r.method): r for r in rows}
    best_per_dataset = {}
    for ds in datasets:
        vals = [rows_by_key[(ds, m)].final_macro_f1 for m in methods if (ds, m) in rows_by_key]
    ...
    lines.append("\\small")
    lines.append("\\caption{Final Macro-F1 ($\\uparrow$) in one run. Stronger gated baselines were calibrated to match the validation-window admitted yield of RAIL-Gated.}")
    lines.append("\\label{tab:main_f1_one_run}")
    ...
    for m in methods:
        ...
        for ds in datasets:
            ...
            txt = _fmt(row.final_macro_f1)
            if abs(row.final_macro_f1 - best_per_dataset[ds]) < 1e-12:
                txt = "\\textbf{" + txt + "}"
            cells.append(txt)
...
def latex_tradeoff_table(rows: Sequence[RunMetrics], datasets: Sequence[str], methods: Sequence[str]) -> str:
    rows_by_key = {(r.dataset, r.method): r for r in rows}
    lines = []
    lines.append("\\begin{table*}[t]")
    lines.append("\\centering")
    lines.append("\\small")
    lines.append("\\caption{Admission trade-off in one run. Lower contaminated admissions is better; higher yield and AE are better.}")
    lines.append("\\label{tab:tradeoff_one_run}")
    ...
    for i, ds in enumerate(datasets):
        ...
        best_cont = min(r.contaminated_admissions for r in subset)
        best_ae = max(r.ae for r in subset)
        for j, m in enumerate(methods):
            row = rows_by_key[(ds, m)]
            ds_cell = f"\\multirow{{{len(methods)}}}{{*}}{{{ds}}}" if j == 0 else ""
            cont = _fmt(float(row.contaminated_admissions), decimals=2)
            yld = _fmt(row.admitted_yield, decimals=3)
            ae = _fmt(row.ae, decimals=3)
            if row.contaminated_admissions == best_cont:
                cont = "\\textbf{" + cont + "}"
            if abs(row.ae - best_ae) < 1e-12:
                ae = "\\textbf{" + ae + "}"
...
def simulate_events(
    ...
    for i in range(len(X)):
        ...
        if overload:
            ...
            delta = float(rng.uniform(*fast_band)) if rng.random() < 0.5 else float(rng.uniform(*slow_band))
...
def make_synthetic_dataset(seed: int) -> DatasetBundle:
    ...
    return DatasetBundle(
        ...
        X_warmup=X_warmup,
        y_warmup=y_warmup,
        validation_events=simulate_events(X_val, y_val, n_classes, rng, 0.35, 0.90, 0.68, (0.1, 0.8), (1.2, 5.5), (7.0, 12.0), (1.5, 4.0), (0.1, 1.3), (1, 3), (0, 1)),
        replay_events=simulate_events(X_rep, y_rep, n_classes, rng, 0.38, 0.89, 0.66, (0.1, 0.8), (1.2, 5.5), (7.0, 12.0), (1.5, 4.0), (0.1, 1.3), (1, 3), (0, 1)),
        X_test=X_test,
        y_test=y_test,
    )
...
def make_secom_like_dataset(seed: int) -> DatasetBundle:
    ...
    return DatasetBundle(
        ...
        X_warmup=X_warmup,
        y_warmup=y_warmup,
        validation_events=simulate_events(X_val, y_val, n_classes, rng, 0.30, 0.91, 0.70, (0.15, 0.9), (1.0, 5.8), (6.8, 11.0), (1.8, 4.3), (0.15, 1.4), (1, 3), (0, 1)),
        replay_events=simulate_events(X_rep, y_rep, n_classes, rng, 0.32, 0.90, 0.69, (0.15, 0.9), (1.0, 5.8), (6.8, 11.0), (1.8, 4.3), (0.15, 1.4), (1, 3), (0, 1)),
        X_test=X_test,
        y_test=y_test,
    )
...
def make_aps_like_dataset(seed: int) -> DatasetBundle:
    ...
    X_rep, y_rep = make_binary_data(rng, 10000, n_features, 0.30, intercept=-2.30, informative_scale=0.22, noise_scale=1.15, missing_prob=0.04)
    ...
    return DatasetBundle(
        ...
        X_warmup=X_warmup,
        y_warmup=y_warmup,
        validation_events=simulate_events(X_val, y_val, n_classes, rng, 0.34, 0.92, 0.72, (0.12, 0.9), (1.1, 5.8), (6.5, 11.5), (1.7, 4.0), (0.15, 1.2), (1, 3), (0, 1)),
        replay_events=simulate_events(X_rep, y_rep, n_classes, rng, 0.36, 0.91, 0.70, (0.12, 0.9), (1.1, 5.8), (6.5, 11.5), (1.7, 4.0), (0.15, 1.2), (1, 3), (0, 1)),
        X_test=X_test,
        y_test=y_test,
    )
...
def make_atc_like_dataset(seed: int) -> DatasetBundle:
    ...
    return DatasetBundle(
        ...
        X_warmup=X_warmup,
        y_warmup=y_warmup,
        validation_events=simulate_events(X_val, y_val, n_classes, rng, 0.28, 0.88, 0.74, (0.10, 0.85), (1.0, 5.2), (6.0, 10.0), (1.6, 4.5), (0.2, 1.4), (1, 4), (0, 2)),
        replay_events=simulate_events(X_rep, y_rep, n_classes, rng, 0.30, 0.87, 0.75, (0.10, 0.85), (1.0, 5.2), (6.0, 10.0), (1.6, 4.5), (0.2, 1.4), (1, 4), (0, 2)),
        X_test=X_test,
        y_test=y_test,
    )
...
def main(seed: int = 7, output_dir: str = "rail_one_run_results") -> Dict[str, object]:
    ...
    for ds in build_all_datasets(seed):
        all_classes = np.unique(np.concatenate([ds.y_warmup, ds.y_test]))
        base_model = SklearnOnlineClassifier(classes=all_classes, random_state=seed)
    ...
    df.to_csv(outdir / "one_run_metrics.csv", index=False)

    main_tex = latex_main_table(all_rows, datasets=datasets, methods=methods)
    trade_tex = latex_tradeoff_table(all_rows, datasets=datasets, methods=methods)

    (outdir / "table_main_f1_one_run.tex").write_text(main_tex, encoding="utf-8")
    (outdir / "table_tradeoff_one_run.tex").write_text(trade_tex, encoding="utf-8")
    (outdir / "one_run_metrics.json").write_text(json.dumps([asdict(r) for r in all_rows], indent=2), encoding="utf-8")
...
if __name__ == "__main__":
    result = main(seed=7, output_dir="rail_one_run_results")
