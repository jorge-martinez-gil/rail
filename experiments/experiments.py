...
def run_decay_curve(
    ...
    return {"policy": policy, "steps": steps, "bal_acc": bal_curve, "macro_f1": f1_curve}
...
def run_dataset_decay_numeric(
    ...
    policies = ["STATIC", "ALWAYS", "GATED", "WEIGHTED"]
    aggs: List[Dict[str, Any]] = []
    for pol in policies:
...
def main():
    ...
    if len(test_pairs) == 0:
        print("WARNING: no ATC test pairs found -- skipping ATC plots.")
    else:
        rail = AtcRailParams(
        ...
        run_atc_decay(train_pairs, test_pairs, rail)
