...
def test_required_horizon_finds_smallest_feasible_n():
    n = required_horizon_for_budget(
        contamination_budget=60.0, confidence=0.95, base_contamination_rate=0.3, false_admission_rate=0.2
    )
    assert n > 0
    eps = 60.0 / n - 0.06
    assert eps > 0
...
def test_required_horizon_returns_minus_one_when_infeasible():
    n = required_horizon_for_budget(
        contamination_budget=0.05, confidence=0.95, base_contamination_rate=0.3, false_admission_rate=0.2
    )
    assert n == -1


def test_monte_carlo_agrees_with_bayes_within_tolerance():
    out = monte_carlo_contamination(
        pi=0.3, alpha=0.2, rho=0.9, horizon=4_000, runs=80, seed=11
    )
...
def test_pareto_envelope_is_minimal_subset():
    ...
    assert all(
        envelope[i].yield_rate <= envelope[i + 1].yield_rate
